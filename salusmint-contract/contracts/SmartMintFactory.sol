...
contract SmartMintFactory is Ownable {
    ...
    event VaultCreated(
        address indexed smartMintVault,
        string name,
        string symbol
    );

    event CreateSalusNFTPool(address indexed smartMintNFT, address indexed smartMintVault, address indexed creator);
    ...
    struct CreateNFTInput {
        ...
        string symbol;
        IERC20 depositToken;
        SmartMintVaultInterface vaultAddress;
    }
    ...
    function createSalusNFTPool(
        InitNFTInput memory initNFTInput,
        CreateVaultInput memory createVaultInput
    ) public returns (address[] memory) {
        address[] memory retAddress = new address[](2);
        // first create vault
        address vaultAddress = createVault(
            createVaultInput.name,
            createVaultInput.symbol,
            createVaultInput.depositToken
        );
        retAddress[0] = vaultAddress;

        // create NFT contract
        address nftAddress = createNFT(
            initNFTInput.name,
            initNFTInput.symbol,
            initNFTInput.depositToken,
            SmartMintVaultInterface(vaultAddress),
            initNFTInput.creator
        );

        retAddress[1] = nftAddress;

        emit CreateSalusNFTPool(nftAddress, vaultAddress, initNFTInput.creator);
        return retAddress;
    }
    ...
    function createNFT(
        ...
        SmartMintNFT smartNFT = new SmartMintNFT(
            _name,
            _symbol,
            _stableCoinAddress,
            _vaultAddress,
    ...
    function createVault(
        ...
        SmartMintVault smartMintVault = new SmartMintVault(
            _stableCoinAddress,
            _name,
            _symbol
        );
