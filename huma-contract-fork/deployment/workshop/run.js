...
async function adjustFees() {
    ...
    const FeeManager = await hre.ethers.getContractFactory("BaseFeeManager");
    const feeManager = FeeManager.attach(deployedContracts["BaseCreditPoolFeeManager"]);

    await feeManager.setFees(
        20_000_000, // flat originatetion fee to be 10 usdc
        5000, // proportional originatetion fee to be 50%
        10_000_000, // flat late fee to be 10 usdc
        5000, // proportional late fee to be 50%
    ...
    const PoolConfig = await hre.ethers.getContractFactory("BasePoolConfig");
    const poolConfig = PoolConfig.attach(deployedContracts["BaseCreditPoolConfig"]);
